...
export class Table extends THREE.Mesh {
    constructor(radius: number, texture: string) {
        ...
        this.material = new THREE.MeshBasicMaterial({
            color: new THREE.Color("saddlebrown"),
        });

        utils.loadGltfMaterialAsync(texture).then((mat) => {
            this.material = mat;
            Room.updateRoom();
        });
    }
    ...
    static CartesianFromAxial(axial: number[]) {
        return [
    ...
    static AxialFromCartesian(cartesian: number[]) {
        return [
    ...
    static TileIdFromCartesian(cartesian: number[]) {
        return this.TileIdFromAxial(this.AxialFromCartesian(cartesian));
    }

    static TileIdFromVector3(vector: THREE.Vector3) {
    ...
    static TileIdFromRingAndSlot(ring: number, slot: number) {
    ...
    getSystem(tileId: number[]) {
        return this.getObjectByProperty("tileId", tileId);
    }

    setSystem(tileId: number[], system:System) {
